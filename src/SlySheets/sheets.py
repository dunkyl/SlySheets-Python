...
RE_A1 = re.compile(
    # like: 'page'!A1:B2
    r"(?:'(?P<page>\w+)'!)?" # TODO: is \w exactly what sheets allows?
...
def sheets_date(timestamp: float, tz: tzinfo) -> datetime:
    '''
...
class Sheet(WebAPI):
    ...
    async def properties(self) -> SheetMetadata:
        ...
        return SheetMetadata(data['title'], data['index'], data['sheetType'], data['tabColor'], data['hidden'], GridProperties(data['gridProperties']['frozenColumnCount'], data['gridProperties']['frozenRowCount'], data['gridProperties']['columnCount']), pytz.timezone(data['timeZone']))
    ...
    async def parse_cell_range(self,
        ...
        match key:
            ...
            case slice(): # row range
                ...
                start_row = key.start
                end_row = key.stop-1 # CellRange in inclusive, python slices are not
            case str() if m := RE_A1.fullmatch(key): # A1 notation
                page = m.group('page') or page
                start_row = maybe(int, m.group('start_row')) or start_row
                end_row = maybe(int, m.group('end_row')) or end_row
    ...
    async def __getitem__(self, *keys: int|str|slice|tuple[int|slice, int|str|slice]) -> Any:
    ...
    #     # reduce dimensions, for convienence
    #     if len(values) == 1 and len(values[0]) == 1:
    #         return values[0][0]
    #     elif len(values) == 1 and not isinstance(key, slice):
    #         return ResultRow(values[0], self)
    ...
    async def append(self, row: CellValue|list[CellValue]|None=None, search_: str = 'A1', **kwargs: CellValue):
        if row is None and not kwargs:
            ...
            if not all(map(lambda x: x in self.headers, kwargs.keys())):
                ...
                p = self.page
                row = [kwargs[key] for key in self.headers[p]]
